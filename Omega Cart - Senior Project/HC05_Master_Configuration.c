...
void UART5_Init(void){
	...
	UART5_IBRD_R = 81;								// IBRD, 50MHz, 38400 baud rate (50MHz / (16 * 38400))
	UART5_FBRD_R = 24;							  // FBRD, (0..3802 * 64 + .05)
...
void UART0_Init(void){
  ...
  UART0_IBRD_R = 325;                    // IBRD = int(50,000,000 / (16 * 9,600)) = int(325.520833)
  UART0_FBRD_R = 33;                     // FBRD = int(0.520833 * 64 + 0.05) = 33
  UART0_LCRH_R = 0x0060;									// 8 bit word length (no parity bits, one stop bit, FIFOs)
...
int main(void){
	...
	UART_transmit_String("HC05 Bluetooth Configuration\r\n");
	
	UART_transmit_String("AT+CMODE=0\r\n");
	sendCommandBT("AT+CMODE=0\r\n");
	respondFromBT();
	Delay();
	
	UART_transmit_String("AT+BIND=14,3,603dd\r\n");
	sendCommandBT("AT+BIND=14,3,603dd\r\n");
	respondFromBT();
	Delay();
	
	UART_transmit_String("AT+UART=9600,1,0\r\n");
	sendCommandBT("AT+UART=9600,1,0\r\n");
	respondFromBT();
	Delay();
	
	UART_transmit_String("AT+ROLE=1\r\n");
	sendCommandBT("AT+ROLE=1\r\n");
	respondFromBT();
	Delay();
