...
x=[x zeros(1,N-length(x))]; %Pad x to length N 
X=dft(x,N); % Find DFT[x(n)]
X=ones(Cm,1)*X; % Make matrix of columns of Xk's 
WN=exp(-2*1j*pi/N); % Get phase factor for shift amount 
k=[0:1:N-1]; 
Y=(WN.^(m' * k)).*X; 
y=real(conj(dft(conj(Y),N)))/N;